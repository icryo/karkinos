...
The actions menu will issue new tasks to [ssh -rm](/agents/karkinos/commands/ssh-rm/) and [ssh -download](/agents/karkinos/commands/ssh-download/) for removing and downloading entries.
This will reuse the credential and host information from the previous task.

![ssh-ls_actions](../images/ssh-ls_actions.png)

An [ssh -cat](/agents/karkinos/commands/ssh-cat/) task can be issued from the browser script
