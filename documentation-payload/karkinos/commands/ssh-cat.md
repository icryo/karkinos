...
#### use ssh agent
 - Option for whether or not to use a connected SSH agent for authentication. Karkinos will use
...
### Browser Script
Cat tasking can be issued through the [ssh -ls](/agents/karkinos/commands/ssh-ls/) browser
script. A `CAT` button next to file entries will issue a new [ssh -cat](/agents/karkinos/commands/ssh-cat/)
task using the path to that file and the credentials/host from that [ssh -ls](/agents/karkinos/commands/ssh-ls/) task.
