...
use crate::{
    bof, cat, cd, cp, download, exit, getenv, getprivs, jobs, ls, mkdir, mv, netstat, portscan,
    ps, pwd, redirect, rm, setenv, shell, sleep, ssh, unsetenv, upload, workinghours,
};
        ...
        if let Some(tasks) = tasks {
            for task in tasks.iter() {
                ...
                match task.command.as_str() {
                    "bof" => {
                        if let Err(e) = self.spawn_background(task, bof::execute_bof, false) {
